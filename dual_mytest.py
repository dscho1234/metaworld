...
from metaworld.envs.mujoco.ur3_xyz.ur3_dual_pick_and_place import UR3PickAndPlaceEnv
import numpy as np
import time
import glfw
...
def sample_pick_and_place():
    ...
    env = UR3PickAndPlaceEnv(random_init=True, tasks = tasks, rotMode='vertical_fixed', hand_init_pos=hand_init_pos)
    ...
    for i in range(100):
        ...
        for _ in range(50):
            ...
            action = np.zeros(env.action_space.flat_dim)
            # action[-1] = -1
            # action = np.zeros(Da)
